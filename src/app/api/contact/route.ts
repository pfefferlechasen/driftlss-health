...
export async function POST(request: Request) {
  try {
    ...
    const { name, email, practice, type, website, message } = body;

    // Validate required fields
    const missing: string[] = [];
    if (!name?.trim()) missing.push("name");
    if (!email?.trim()) missing.push("email");
    if (!practice?.trim()) missing.push("practice");
    ...
    await resend.emails.send({
      ...
      subject: `New inquiry from ${practice.trim()}`,
      text: [
      ...
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
          <h2 style="color: #0d9488;">New Inquiry from ${practice.trim()}</h2>
          <table style="width: 100%; border-collapse: collapse;">
            ...
            <tr>
              ...
              <td style="padding: 8px 12px; color: #6b7280; border-bottom: 1px solid #e5e7eb;">${type.trim()}</td>
            </tr>
            <tr>
